...
FREE_IMAGE_FORMAT GetInputImageFormat(std::string& sFilename)
{
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		if (checkCmdLineFlag(argc, (const char**)argv, "input")) {
			...
			filePath = sdkFindFilePath("5.1.09.tiff", argv[0]);
		}
		...
		if (checkCmdLineFlag(argc, (const char**)argv, "lt")) {
			int tmp;
			tmp = getCmdLineArgumentInt(argc, (const char**)argv, "lt");
			nLowThreshold = tmp;
		}


		Npp16s nHighThreshold = 256;
		if (checkCmdLineFlag(argc, (const char**)argv, "ht")) {
			int tmp;
			tmp = getCmdLineArgumentInt(argc, (const char**)argv, "ht");
			nHighThreshold = tmp;
		}
		...
		switch (GetInputImageFormat(sFilename))
		{
		case FIF_BMP:
			npp::saveImageWithFormat<FIF_BMP>(sResultFilename, oHostDst);
			std::cout << "Saved image: " << sResultFilename << std::endl;
			break;
		case FIF_PNG:
			npp::saveImageWithFormat<FIF_PNG>(sResultFilename, oHostDst);
			std::cout << "Saved image: " << sResultFilename << std::endl;
			break;
		case FIF_TIFF:
			npp::saveImageWithFormat<FIF_TIFF>(sResultFilename, oHostDst);
			std::cout << "Saved image: " << sResultFilename << std::endl;
			break;
		default:
			std::cout << "Error Saving image: Unknown input file format!" << std::endl;
			break;
		}
