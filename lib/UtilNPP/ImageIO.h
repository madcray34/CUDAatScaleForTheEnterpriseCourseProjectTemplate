...
namespace npp
{
    ...
    // Save an gray-scale image to disk.
    template <FREE_IMAGE_FORMAT Format>
    void
    saveImageWithFormat(const std::string &rFileName, const ImageCPU_8u_C1 &rImage)
    {
        // create the result image storage using FreeImage so we can easily
        // save
        FIBITMAP *pResultBitmap = FreeImage_Allocate(rImage.width(), rImage.height(), 8 /* bits per pixel */);
        ...
        // now save the result image
        bool bSuccess;
        bSuccess = FreeImage_Save(Format, pResultBitmap, rFileName.c_str(), 0) == TRUE;
        NPP_ASSERT_MSG(bSuccess, "Failed to save result image.");
    }
